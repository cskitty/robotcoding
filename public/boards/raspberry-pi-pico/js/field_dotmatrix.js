...
CustomFields.FieldDotMatrix.prototype.renderEditor_ = function(firsttimes) {
  ...
  for (let y=0;y<5;y++) {
    for (let x=0;x<5;x++) {
      this.editor_.querySelector(`input[data-n='${x},${y}']`).checked = (value[x] & (0x10 >> y)) !== 0;
    }
  }
...
CustomFields.FieldDotMatrix.prototype.showEditor_ = function() {
  ...
  html += `<div class="menu">`;
  html += `<div class="custom active">定制</div>`;
  html += `<div class="collection">预设</div>`;
  html += `</div>`;
...
CustomFields.FieldDotMatrix.prototype.createImageDotMatrix_ = function(value) {
  ...
  for (let y=0;y<5;y++) {
    for (let x=0;x<5;x++) {
      ...
      ctx.fillStyle = (value[x] & (0x10 >> y)) !== 0 ? "#FF0000" : "#FFFFFF";
      ctx.fill();
    }
  }
