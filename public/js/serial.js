...
let serialConnectWeb = async () => {
  ...
  NotifyS(yy_msg['SerialConnected'])
...
let serialConnectElectron = async (
  ...
  if (!portName) {
    try {
      ...
      NotifyE(yy_msg['SerialNotConnected'])
      console.log(e)
      return false
    }
  }
  ...
  try {
    ...
    if (!autoConnect) NotifyE(yy_msg['SerialCanNot'])
    console.log(e)
    serialPort = null
...
$('#upload-program').click(async function () {
  ...
  if (isInYYBrowser == 1) {
    var r = UploadCode($('#project-name').val(), code)
    if (r == 1) {
      NotifyS('上传成功')
    }
    $('#upload-program').removeClass('loading')
...
let findIncludeModuleNameInCode = (code) => {
  ...
  moduleList = moduleList.filter(
    (moduleName) => moduleBuiltIn.indexOf(moduleName) < 0
  )
...
$('#disconnect-device').click(async () => {
  if (serialPort) {
    ...
    if (!isElectron) {
      ...
      // if (await NotifyConfirm(yy_msg['SerialCloseConfirm'])) {
      //     window.close()
      // }
    } else {
      serialPort.close()
