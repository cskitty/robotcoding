...
let updateBlockCategory = async () => {
  ...
  for (let category of toolboxTree) {
    toolboxTextXML += `<category name="${category.name}" colour="${
      category.color
    }"${
      typeof category.blocks === "string" ? ` custom="${category.blocks}"` : ""
    }>`;
    ...
    categoryIconList.push(
      category.icon.startsWith("/")
        ? rootPath + category.icon
        : `${rootPath}/boards/${boardId}/${category.icon}`
    );
  }
  ...
  for (const extensionId of fs.ls("/extension")) {
    ...
    categoryIconList.push(
      fs.read(`/extension/${extensionId}/${extension.icon}`)
    );
  }
  if (isElectron) {
    let extensionDir = sharedObj.extensionDir;
    for (const extensionId of nodeFS.ls(extensionDir)) {
      let extension = await readFileAsync(
        `${extensionDir}/${extensionId}/extension.js`
      );
...
if (isEmbed) {
  ...
  $("#embed-edit").click(() => {
    window.open(
      `https://127.0.0.1//?open=${pageParams.get("open") || ""}`,
      "_blank"
    );
  });
...
Blockly.prompt = function (message, defaultValue, callback) {
  Notiflix.Confirm.Show(
    yy_msg["Prompt"],
    `${message}<br><br><input type="text" id="prompt-input" value="${defaultValue}">`,
    yy_msg["Confirm_YES"],
    yy_msg["Confirm_NO"],
    function () {
...
if (isElectron) {
  ...
  nodeFS.ls = (dir) =>
    nodeFS
      .readdirSync(dir)
      .filter((f) => nodeFS.statSync(path.join(dir, f)).isDirectory());
}
  ...
  for (const extensionId of fs.ls("/extension")) {
    ...
    for (const file of blocksFile) {
      if (file.endsWith(".js")) {
        ...
        console.warn(
          "Why file " + file + " in blocks ? support .js only so skip it"
        );
      }
    }
  }
  ...
  if (openArg) {
    (async (fileName) => {
      ...
      let fileContent = await fetch(
        `https://us-central1-ublock-c0a08.cloudfunctions.net/share/files/${fileName}`,
        {
          method: "get",
          redirect: "follow",
        }
      ...
      if (isEmbed) {
        ...
        if (updateFrameSize) {
          ...
          if (fSizeW === 0)
            fSizeW = `${width + 20 + (embedOption.blockOnly != 1 ? 20 : 0)}px`;
          if (fSizeH === 0)
            fSizeH = `${height + 20 + (embedOption.blockOnly != 1 ? 60 : 0)}px`;
...
let saveCodeToLocal = () => {
  if (useMode === "block") {
    fs.remove("/main.py");
    try {
      var xmlText = Blockly.Xml.domToPrettyText(
...
$("#save-project").click(async () => {
  if (isInYYBrowser == 1) {
    SaveProject($("#project-name").val(), JSON.stringify(vFSTree));
    return;
  }
  ...
  if (!isElectron) {
    ...
    if (!projectFilePath || saveAsFlag) {
      let result = await dialog.showSaveDialog({
        filters: [
          {
            name: "Windtree Robotics Block IDE",
            extensions: ["mby"],
          },
        ],
...
let openProject = async (filePath) => {
  if (!(await NotifyConfirm(yy_msg["OpenProjConfirm"]))) {
    return;
  }
...
$("#save-code").click(async () => {
  ...
  download($("#project-name").val() + ".py", code);
});
