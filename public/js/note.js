...
const saveNote = e => {
    if (e) {
        $(e.target).parents(".note-box-main").find(".sub-header").text(yy_msg["NoteEditedOn"] + new Date().toLocaleString());
...
const updateEvent = () => {
    ...
    $("#note-box-list .note-box-main > header > div").on("focus", e => {
        console.log("focus");
        if (e.target.innerHTML === `<i>${yy_msg["NoteTitle"]}</i>`) {
            e.target.innerHTML = "";
        }
    }).on("blur", e => {
        console.log("blur");
        if (e.target.innerHTML === "<br>" || e.target.innerHTML.length === 0) {
            e.target.innerHTML = `<i>${yy_msg["NoteTitle"]}</i>`;
        }
    }).on("input", e => {
    ...
    $("#note-box-list .note-box-main > article > div").on("focus", e => {
        if (e.target.innerHTML === `<i>${yy_msg["NoteContent"]}</i>`) {
            e.target.innerHTML = "";
        }
    }).on("blur", e => {
        if (e.target.innerHTML === "<br>" || e.target.innerHTML.length === 0) {
            e.target.innerHTML = `<i>${yy_msg["NoteContent"]}</i>`;
        }
    }).on("input", e => {
        // console.log(e.target.innerHTML);
        saveNote(e);
        ...
        html += `
                        <li data-id=${note_item?.id || ""}>
                            <div class="note-box-main ${note_item?.color}">
                                <header><div contenteditable="true" spellcheck="false">${note_item?.title || "<i>${yy_msg['NoteTitle']}</i>"}</div></header>
                                <div class="sub-header">${yy_msg["NoteEditedOn"]} ${new Date(note_item.last_edit).toLocaleString()}</div>
                                <article><div contenteditable="true" spellcheck="false">${note_item?.content || "<i>${yy_msg['NoteContent']}</i>"}</div></article>
                                <footer>
                                    <ul class="color-list">
    ...
    $("#note-box-list").prepend(`
                        <li data-id="${now_time}" data-last-edit="${now_time}">
                            <div class="note-box-main yellow">
                                <header><div contenteditable="true" spellcheck="false"><i>${yy_msg["NoteTitle"]}</i></div></header>
                                <div class="sub-header">${yy_msg["NoteCreateOn"]} ${new Date().toLocaleString()}</div>
                                <article><div contenteditable="true" spellcheck="false"><i>${yy_msg["NoteContent"]}</i></div></article>
                                <footer>
                                    <ul class="color-list">
                                        <li><span class="yellow"></span></li>
                                        <li><span class="green active"></span></li>
